...
class VBApi {
  ...
  constructor(apiUrl, apiKey, platformName, platformVersion) {
    ...
    this.__connectionVars = {
      ...
      uniqueId: md5(
        "nodeVBulletinAPI" + version + platformName + platformVersion + uuidV1()
      ),
    };
    ...
    if (
      ...
      this.__clientSessionVars.error =
        "apiInit(): Initialization requires a `apiUrl`, `apiKey`, `platformName`, and `platformVersion`";
      ...
      try {
        ...
        if (
          ...
          that.__clientSessionVars = {
            ...
            sessionHash: null, //response.sessionhash,
        ...
        if (result === null) {
          that.__clientSessionVars.error =
            that.constructor.parseErrorMessage(response) ||
            "TODO ERROR (api connection did not return a session)";
        ...
        that.__waitingForInitializationTimeout = setTimeout(
          function () {
            ...
            if (that.initialized) {
              ...
              reject(
                "Connection could not be achieved due to timed out",
                that.__clientSessionVars.error
              );
            }
          },
        ...
        if (sign === true) {
          ...
          if (that.initialized) {
            reqParams.api_sig = md5(
              that.__clientSessionVars.apiAccessToken +
                that.__clientSessionVars.apiClientId +
                that.__clientSessionVars.secret +
                that.__connectionVars.apiKey
            );
          } else {
    ...
    return new Promise(async function (resolve, reject) {
      try {
        ...
        if (response.sessionhash) {
          ...
          // }
          that.userSessionVars.dbsessionhash = response.sessionhash;
          that.userSessionVars.userid = response.userid;
          that.userSessionVars.username = options.username;
          that.userSessionVars.loggedIn = true;
